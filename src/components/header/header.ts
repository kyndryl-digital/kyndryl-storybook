...
export class kdHeader extends LitElement {
  ...
  _init() {
    for (const e of this.mainNavItems) {
      ...
      null !== t &&
        (t.setAttribute('aria-hidden', 'true'), this.changeIndex(t, -1));
    }
  }
  ...
  _bindEvents() {
    ...
    for (const e of this.mainNavItems) {
      if (
        (e.addEventListener('keydown', this.toggleMenu.bind(this)),
      ...
      } else e.addEventListener('focus', this.closeAllMenu.bind(this));
      e.parentNode.addEventListener('click', () => {
        e.classList.contains('cmp-navigation__item-link--active')
          ? this.closeMenu(e)
          : (this.closeAllMenu(), this.openMenu(e));
      });
    }
    for (const e of this.mobileNavItems)
      e.addEventListener('click', t => {
        t.preventDefault();
        t.target.classList.contains('cmp-navigation__item-link--active')
          ? this.closeMenu(e)
          : this.openMenu(e);
      });
  ...
  toggleMenu(e) {
    if ('Space' === e.code) {
      e.preventDefault();
      const t = e.target;
      t.classList.contains('cmp-navigation__item-link--active')
        ? this.closeMenu(t)
        : this.openMenu(t);
    }
  }
  ...
  closeAllMenu() {
    ...
    if (e.length) for (const t of e) this.closeMenu(t);
  }

  changeIndex(e, t) {
    const i = e.querySelectorAll('a');
    for (const e of i) e.tabIndex = t;
  }
