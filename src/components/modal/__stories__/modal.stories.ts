...
export const Button = (args) => {
  return html`
      <kd-modal size="${args.size}">
          <kd-button slot="trigger" type="primary" icon="pop-out" href="javascript://">${args.label}</kd-button>
          <div class="kd-pt4 kd-pr4 kd-pl4 kd-pb6">${args.content}</div>
      </kd-modal>`;
};
...
export const Image = (args) => {
  return html`
      <kd-modal size="${args.size}">
          <img slot="trigger" src="https://placehold.co/400x300?text=${args.label}" alt=${args.label} title=${args.label} />
          <div class="kd-pt4 kd-pr4 kd-pl4 kd-pb6">${args.content}</div>
      </kd-modal>`;
};
