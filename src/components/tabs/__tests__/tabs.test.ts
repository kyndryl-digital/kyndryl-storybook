...
describe('tabs', () => {
  ...
  describe('check main tab', () => {
    it('select Dolor tab', async () => {
    const label = document.body.getElementsByTagName(COMPONENT_TAG)[0].shadowRoot.textContent;
    expect(label).toContain('Dolor');
    });

    it('Now check whether Dolor it is active or not', () => {
      const label = document.body.getElementsByTagName(COMPONENT_TAG)[0].getElementsByTagName(child_tag)[0].getAttribute('selected');
       expect(label).toContain('true');
    });

    it('Ipsum tab should not be active', () => {
      const label = document.body.getElementsByTagName(COMPONENT_TAG)[0].getElementsByTagName(child_tag)[1].getAttribute('selected');
